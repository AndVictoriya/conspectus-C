...
//Функции
//Любая функция должна быть объявлена перед тем, как будет использована. Если описание функции находится до ее первого вызова,
//то прототип не нужен.
//Но если описание находится где-то дальше, то нужно все-таки дать программе понять, что такая функция существует. 
//Для этого и нужен прототип.

float FUNC(int, double); 
//Так же прототип соответствует списку параметров
